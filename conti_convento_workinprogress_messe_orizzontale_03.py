...
canvas = Canvas(Frame_pandastable, bg="black", height=400, width=800)
canvas.grid()
text = canvas.create_text((300,150),text=df, fill='white')


# label_frame = LabelFrame(canvas, text="Geeks For Geeks")
# label = Label(label_frame, text=df)
# label.pack()
...
def clicked():
    ...
    df = df.loc[(df['Anno']==Entry_Anno_pandastable_IntVar.get()) &
                (df['Mese']==Entry_Mese_pandastable_StringVar.get())
               ]
...
# def aggiorna_pandas_Table():
        ...
        #            ]
        #
        # print(Entry_Mese_pandastable_StringVar.get())
        # print(Entry_Anno_pandastable_IntVar.get())

        # canvas = Canvas(Frame_pandastable, bg="yellow", height=200, width=400)
        #
        # canvas.grid()
        # canvas.create_text(300, 50, text=df, fill="black", font=('Helvetica 5 bold'))
        # canvas.grid()
        ...
        # id=canvas.create_window(0, 100, window=label_frame, anchor='w')
        # # pt = Table(Frame_pandastable, dataframe=df, width=100, height=100)
        # # pt.textcolor = 'blue'
        # # pt.redraw()
        # canvas.itemconfig(id, label(text=df))
...
Entry_Anno_pandastable_IntVar                 = IntVar()
Entry_Mese_pandastable_StringVar              = StringVar()
#df.drop(columns=['ID'])
# List Anni
Anni = [2020,2021,2022,2023,2024,2025,2026,2027,2028,2029,2030]

# Dropbox Anno
Entry_Anno_combo = ttk.Combobox(Frame_pandastable, font=("Helvetica", 10), values=Anni, textvariable=Entry_Anno_pandastable_IntVar)
Entry_Anno_combo.current(4)
Entry_Anno_combo.grid(row=0, column=1)
Entry_Anno_combo['state'] = 'readonly'
...
# Dropbox Mesi
Entry_Mese_combo = ttk.Combobox(Frame_pandastable, font=("Helvetica", 10), values=Mesi, textvariable=Entry_Mese_pandastable_StringVar)
Entry_Mese_combo.current(0)
Entry_Mese_combo.grid(row=0, column=2)
Entry_Mese_combo['state'] = 'readonly'
...
B_pandatable = Button(Frame_tree_Buttons, text='pandasTable', width=10, command=clicked).pack(side=BOTTOM, pady=20)
